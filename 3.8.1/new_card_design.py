...
async def fetch_gift_data(gift_name, force_fresh=False):
    """Fetch gift data using Tonnel API for premarket gifts, Portal API for others."""
    try:
        ...
        if premarket_key:
            ...
            if price_ton:
                ...
                price_usd = price_ton * 3.0  # Approximate TON-to-USD conversion
                return {
                    "name": gift_name,
                    "priceTon": price_ton,
                    "priceUsd": price_usd,
...
async def fetch_chart_data(gift_name, force_fresh=False):
    """Fetch chart data using Legacy API for premarket gifts, Portal API for others."""
    try:
        # Check if this is a premarket gift
        import tonnel_api
        # Check if gift_name is one of the values in PREMARKET_GIFTS (display names)
...
def generate_template_card(gift_name):
    ...
    try:
        ...
        watermark_color = (255, 255, 255, 180)  # Semi-transparent white
        watermark_width = draw.textlength(watermark_text, font=watermark_font)
        watermark_x = template.width // 2 - watermark_width // 2  # Centered horizontally
        watermark_y = 40  # 40px from top edge
        draw.text((watermark_x, watermark_y), watermark_text, fill=watermark_color, font=watermark_font)
