...
def generate_price_card(collection, sticker, price, output_dir):
    """Generate a price card for a sticker using the template and metadata"""
    try:
        ...
        if not metadata:
            logger.warning(f"No metadata found for {collection} - {sticker}")
            return None
        ...
        dominant_color = tuple(metadata.get('dominant_color', (148, 68, 143)))
        ...
        dollar_x = metadata.get('dollar_x', 155)
        dollar_y = metadata.get('dollar_y', 285)
        ton_x = metadata.get('ton_x', 215)
        ton_y = metadata.get('ton_y', 465)
        star_x = metadata.get('star_x', 515)
        star_y = metadata.get('star_y', 465)
